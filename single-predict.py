...
def custom_run_epoch(
    ...
    save_path = os.path.join(
        output_dir,
        "output_subject{}_fold_{}.npz".format(subject_idx,fold_idx)
    )
...
def predict(
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        for fold_idx in range(5):
            ...
            #if fold_idx==4:
            #    n_subjects_per_fold=49
            for i in range(n_subjects_per_fold):
                #if fold_idx==3 and i==6:
                #    continue
                #if fold_idx==3 and i==7:
                #    continue

                # Load testing data
                x, y = SeqDataLoader.load_single_subject_data(
                    data_dir=data_dir,
                    subject_idx=(fold_idx*n_subjects_per_fold)+i
                )
