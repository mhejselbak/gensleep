...
tf.app.flags.DEFINE_float('z_prob', 0.1,
                            """Define zoneout prob.""")
tf.app.flags.DEFINE_integer('dat_set', 1,
                            """Define multi annotator.""")
...
def ensemble(y1,y2,y3,y4,y_true_,fold_idx,sub_idx):
    ...
    if sub_idx<10:
        save_path = os.path.join(
            FLAGS.output_dir,
            "output_fold_{}_subj_0{}.npz".format(fold_idx,sub_idx)
        )
    else:
        save_path = os.path.join(
            FLAGS.output_dir,
            "output_fold_{}_subj_{}.npz".format(fold_idx,sub_idx)
        )
    ...
    for fold_idx in range(5):
        ...
        for i in range(n_subjects_per_fold):
            y_pred, y_true = predict(
                data_dir="/Glostrup/100-C3-M2-data/",
                model_dir="/Glostrup/100-C3-M2-model",
                output_dir="/Glostrup/100-C3-M2-model",
            ...
            y_pred2 ,_ = predict(
                data_dir="/Glostrup/100-F3-M2-data/",
                model_dir="/Glostrup/100-F3-M2-model",
                output_dir="/Glostrup/100-F3-M2-model",
                n_subjects_per_fold=n_subjects_per_fold,
                fold_idx=fold_idx,
                sub_idx=i,
                hid_rnn_size=512
            ...
            y_pred3 ,_ =  predict(
                data_dir="/Glostrup/100-O1-M2-data/",
                model_dir="/Glostrup/100-O1-M2-model",
                output_dir="/Glostrup/100-O1-M2-model",
                n_subjects_per_fold=n_subjects_per_fold,
                fold_idx=fold_idx,
                sub_idx=i,
                hid_rnn_size=512
            ...
            y_pred4 ,_ =  predict(
                data_dir="/Glostrup/100-E1-M2-data/",
                model_dir="/Glostrup/100-E1-M2-model",
                output_dir="/Glostrup/100-E1-M2-model",
                n_subjects_per_fold=n_subjects_per_fold,
                fold_idx=fold_idx,
                sub_idx=i,
                hid_rnn_size=512
            ...
            y_ = ensemble(y_pred,y_pred2,y_pred3,y_pred4,y_true,fold_idx,i)
