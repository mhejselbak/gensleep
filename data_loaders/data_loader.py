...
class NonSeqDataLoader(object):
    ...
    def load_train_data(self, n_files=None):
        ...
        fold_len = len(allfiles) / self.n_folds
        for i in range(fold_len*self.fold_idx,fold_len*(self.fold_idx+1)):
            subject_files.append(npzfiles[i])
...
class SeqDataLoader(object):
    ...
    @staticmethod
    def load_subject_data(data_dir, subject_idx, n_folds):
        # Remove non-mat files, and perform ascending sort
       ...
       #         subject_files.append(os.path.join(data_dir, f))
        ...
        allfiles.sort()
        fold_len = len(allfiles) / n_folds
        #fold_len = len(allfiles) / 5
        for i in range(fold_len*subject_idx,fold_len*(subject_idx+1)):
            subject_files.append(os.path.join(data_dir, allfiles[i]))
        # Files for validation sets
    ...
    @staticmethod
    def load_single_subject_data(data_dir, subject_idx):
        # Remove non-mat files, and perform ascending sort
        allfiles = os.listdir(data_dir)
        subject_files = []
